...
This release transforms RavenForge from a plain build utility into a living
forge. The title pulses in Viking gold, runes drift across the interface, the
Vegvisir compass turns, and every build step glows with animated feedback.
A critical frozen-exe re-launch bug is also eliminated.

---

### 🔤 Viking Regular Title Font

RavenForge scans common font directories at startup and registers `Viking.ttf`
via Windows GDI if found. The title renders at **36pt bold Viking Regular**,
pulsing through a gold keyframe sequence every 40 ms. Place `Viking.ttf`
alongside the script or in `C:\Windows\Fonts` — picked up automatically.
Falls back to Palatino Linotype → Book Antiqua → Georgia when absent.

---

### ᛟ Animated Vegvisir Compass

The static `᛭` glyph has been replaced with a fully animated `tk.Canvas`
Vegvisir compass:
- A 16-rune ring rotates the full Elder Futhark around the outer edge
- Eight alternating gold / purple spokes pulse with sine-wave brightness
- The outer ring breathes between dim and glow purple
- The centre `᛭` glyph glows gold on its own slower cycle

---

### ᚱ Rune Particle Field + GlowLine Borders

A drifting Elder Futhark particle strip scrolls just below the title bar —
each particle on an independent sine-wave phase, ambient and non-intrusive.

Two sweeping gradient lines bookend the title bar, cycling through
`PURPLE → GOLD → PURPLE_GL` (top) and `GOLD → GOLD_BR → PURPLE_GL` (bottom)
at offset speeds.

---

### ᚹ Icon Chooser with Live Preview

The icon picker now shows a 64×64 canvas preview of the selected icon:
- PIL renders a real image thumbnail for `.ico`, `.png`, `.jpg`, `.bmp`
- An animated rune placeholder breathes purple when no icon is set
- A `✕ clear` button removes the current selection

---

### ⚒ Animated Build Feedback

| Element | Animation |
|---|---|
| **⚒ FORGE button** | Gold breathe when idle; dims during build |
| **Active step rows** | Cyan pulse while running |
| **Completed steps** | Green lock-in |
| **Failed steps** | Red lock-in |
| **Console header** | Cycling rune ticker |

---

### 🐛 Critical Bug Fixed — BUG-F01: Frozen .exe Re-Launch Loop

When RavenForge was packaged with PyInstaller, every `pip install` and
`pyinstaller` subprocess re-launched `ravenforge.exe` instead of calling
`python.exe`. This caused a cascade of new RavenForge windows and failed builds.

**Fix:** `PYTHON_EXE` is now resolved at startup:
- Running as `.py` → `sys.executable` (real Python)
- Running as `.exe` → `shutil.which("python")` (system Python)

All subprocess calls use `PYTHON_EXE`.
