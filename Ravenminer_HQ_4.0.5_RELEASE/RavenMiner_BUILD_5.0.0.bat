...
echo.
echo %PGLW% ========================================================================%RST%
echo %GBRT%  RAVENMINER HQ -- BUILD FORGE -- v4.0.0 through v5.0.0%RST%
...
rem -----------------------------------------------------------------------
rem AUTO-LOCATE source .py  (v4.0.0 through v5.0.0)
...
echo %CDIM%       Supported range: v4.0.0 -- v5.0.0%RST%
...
pip install "requests>=2.32.3" "Pillow>=10.4.0" "pystray>=0.19.5"
echo.
...
rem -----------------------------------------------------------------------
rem Shared PyInstaller flags -- single definition
rem -----------------------------------------------------------------------
set "PYI_COMMON=--windowed --name RavenMinerHQ --distpath "%DIST_DIR%" --workpath "%WORK_DIR%" --specpath "%WORK_DIR%" --hidden-import=PIL._tkinter_finder --hidden-import=pystray --hidden-import=pystray._win32 --hidden-import=requests --collect-all=PIL"
...
color 0D
python -m PyInstaller %BUILD_MODE% %PYI_COMMON% %ICON_FLAG% "%SRC%"
